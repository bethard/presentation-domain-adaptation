...
\newcommand{\randval}{\pgfmathparse{random(0,99)/10}\pgfmathprintnumber[precision=1,fixed,fixed zerofill]{\pgfmathresult}}
\newcommand{\randvec}{[\randval{} \randval{} \ldots]}
...
\draw[ultra thick, -latex,align=center]  (target-docs) --  node[auto,swap,pos=0.8] {Select $k$ \\ by XX} (human);
...
Results:
\begin{itemize}
\item +160 sentences yields +8.6 $F_1$ on time expressions
...
\node<2->[ua-red] (text-vec) at (2.3, 0) {\small\randvec}
  edge[ua-red, ultra thick, latex-] (text);
\foreach[count=\i from 0, count=\slide from 4] \term/\vec/\score in {
    vertigo\\(C0042571)/\randvec/0.7,
    spinning gait\\(C0427152)/\randvec/0.1,
    head\\(C0018670)/\randvec/0.2,
    dizziness\\(C0012833)/\randvec/0.9} {
