...
Results:
\begin{itemize}
\item XX gain on negation, YY gain on time expressions
\end{itemize}
\end{frame}
...
\node (provided-model) at (4, 1.4) [ml model=0.8, label={Provided Model}];

\node[doc,label={Target Domain}] (target-docs) at (-2, 1.4) {?};

\node (human) at (-2, -1.4)  {\includegraphics[scale=0.5]{female-user.pdf}};

\draw[ultra thick, -latex,align=center]  (target-docs) --  node[auto,swap,pos=0.75] {Select $k$ \\ by XX} (human);
...
\node[data,anchor=west] (target-labels) at (0, -1.4) {
\setlength{\tabcolsep}{0.5em}
\begin{tabular}{@{} c @{}}
\toprule
Labels  \\
...
\node (st-model) at (4, -1.4)  [ml model=0.8, label=below:{Adapted Model}];
...
Results:
\begin{itemize}
\item Selecting only 20 yields YY gain on time expressions
